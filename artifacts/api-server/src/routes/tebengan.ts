...
router.get("/tebengan/:id", async (req, res): Promise<void> => {
  ...
  res.json({
    ...
    driver: row.driver ? { id: row.driver.id, nama: row.driver.nama, no_whatsapp: isMitra ? null : row.driver.no_whatsapp, foto_profil: row.driver.foto_profil ?? row.driver.foto_diri ?? null } : null,
