  ...
  const results = rows
    .map(({ s, driver, kendaraan }) => {
      ...
      return {
        ...
        driver: driver ? { id: driver.id, nama: driver.nama, foto_profil: driver.foto_profil ?? driver.foto_diri ?? null } : null,
        kendaraan: kendaraan
...
router.get("/schedules/:id", async (req, res): Promise<void> => {
  ...
  res.json({
    ...
    waypoints,
    driver: row.driver ? { id: row.driver.id, nama: row.driver.nama, foto_profil: row.driver.foto_profil ?? row.driver.foto_diri ?? null } : null,
    ...
    rows.map(({ b, s, driver, kendaraan }) => {
      ...
      return {
        ...
        schedule: s
          ...
          : null,
        driver: driver
          ? { id: driver.id, nama: driver.nama, no_whatsapp: driver.no_whatsapp, foto_profil: driver.foto_profil ?? driver.foto_diri ?? null }
          : null,
        kendaraan: kendaraan
          ? {
              id: kendaraan.id,
  ...
  res.json({
    ...
    driver: row.driver
      ? {
          ...
          foto_profil: row.driver.foto_profil ?? row.driver.foto_diri ?? null,
        }
      : null,
    kendaraan: row.kendaraan
      ? {
