...
router.get("/carter/search", async (req, res): Promise<void> => {
  ...
  const rows = await db
    .select({
      ...
      driver_foto_diri: usersTable.foto_diri,
  ...
  const result = filtered.map((r) => ({
    ...
    driver: { id: r.driver_id, nama: r.driver_nama, foto_profil: r.driver_foto_profil ?? r.driver_foto_diri ?? null },
    kendaraan: {
      id: r.kendaraan_id,
...
router.get("/carter/mitra/:settings_id", async (req, res): Promise<void> => {
  ...
  res.json({
    ...
    driver: driver
      ? { id: driver.id, nama: driver.nama, foto_profil: driver.foto_profil ?? driver.foto_diri ?? null }
      : null,
    kendaraan: kendaraan
      ? {
          id: kendaraan.id,
...
async function loadCarterBookingDetail(bookingId: number, currentUserId?: number) {
  ...
  return {
    ...b,
    driver: driver
      ? { id: driver.id, nama: driver.nama, foto_profil: driver.foto_profil ?? driver.foto_diri ?? null, no_whatsapp: driver.no_whatsapp ?? null }
      : null,
    penumpang: penumpang
