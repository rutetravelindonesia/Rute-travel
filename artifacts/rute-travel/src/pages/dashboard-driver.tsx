...
export default function DashboardDriver() {
  ...
  const [fotoUrl, setFotoUrl] = useState<string | null>(
    user?.foto_profil ?? user?.foto_diri ?? null
  );
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/users/me`, { headers: { Authorization: `Bearer ${token}` } })
      .then((r) => r.json())
      .then((data) => {
        const foto = data?.foto_profil ?? data?.foto_diri ?? null;
        if (foto) { setFotoUrl(foto); setFotoError(false); }
      })
      .catch(() => {});
  }, [token, apiBase]);
      ...
      >
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            {/* Avatar foto profil */}
            <button onClick={() => setLocation("/profil")} className="flex-shrink-0">
