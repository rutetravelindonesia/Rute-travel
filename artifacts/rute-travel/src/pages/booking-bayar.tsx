...
import { ArrowLeft, Building2, Copy, Check, QrCode, Wallet, Upload, Loader2, ImageIcon } from "lucide-react";
...
interface Booking {
  ...
  status: string;
  schedule: { id: number; origin_city: string; destination_city: string; departure_date: string; departure_time: string } | null;
...
export default function BookingBayar() {
  const [, setLocation] = useLocation();
  const [, params] = useRoute("/booking/:id/bayar");
  ...
  const handleFile = async (file: File | null) => {
    ...
    try {
      ...
      const proofRes = await fetch(`${apiBase}/bookings/${booking.id}/payment-proof`, {
        method: "POST",
      ...
      setLocation(`/booking/${booking.id}/etiket`);
    } catch (e: unknown) {
    ...
    <div className="min-h-screen bg-background max-w-md mx-auto pb-8">
      <div className="bg-card border-b border-border px-5 pt-10 pb-4 flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-base font-bold text-foreground">Pembayaran</h1>
          <p className="text-xs text-muted-foreground">
            Booking #{booking.id} · {booking.schedule?.origin_city} → {booking.schedule?.destination_city}
          </p>
        </div>
      </div>
      ...
      <div className="px-5 pt-4 space-y-4">
        ...
        <div className="bg-accent text-white rounded-2xl p-4">
          <p className="text-[10px] font-bold tracking-widest uppercase opacity-80">Total Tagihan</p>
          ...
          <div className="mt-3 pt-3 border-t border-white/20 flex items-center justify-between text-[11px]">
            <span>{booking.kursi.length} kursi · #{booking.kursi.join(", ")}</span>
            {booking.schedule && <span>{longDate(booking.schedule.departure_date)} · {booking.schedule.departure_time}</span>}
          </div>
        </div>
        ...
        {method === "transfer" && (
          <div className="bg-card rounded-2xl border border-border p-4 space-y-3" data-testid="transfer-detail">
            ...
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              Transfer tepat <span className="font-bold text-foreground">{formatRupiah(booking.total_amount)}</span> ke rekening di atas. Setelah transfer, unggah bukti transfer di bawah agar e-tiket otomatis terbit.
            </p>
          </div>
        )}
